...
const fetchProductDirectBuy = async ({ queryKey }: any) => {
  const [, customerId, token] = queryKey;
  const res = await fetch(`${apiUrl}/api/transactions/getProductDirectBuy`, {
    method: "GET",
    headers: {
...
const PaymentDirectBuyScreen = () => {
  ...
  const { data, isLoading, refetch, isRefetching } = useQuery({
    queryKey: ["getProductDirectBuy", customerId, customerDetails?.token],
    queryFn: fetchProductDirectBuy,
  ...
  const getTotal = () => {
    return data?.data?.reduce((sum, item) => sum + item.price * item.qty, 0);
  };
...
export default PaymentDirectBuyScreen;
